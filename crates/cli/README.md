...
Login requests only an IAM short-lived token. A Silicon must configure its
Hook-issued destination with `remind webhook set <url>` before creating reminders.
...
rotating sessions and sandbox keys are stored under `~/.remind/` with restrictive
permissions. Use `--test <id>` before ordinary commands for an isolated sandbox.

Complete API, CLI, client, IAM, Hook and testing guides are included in `docs/`.
