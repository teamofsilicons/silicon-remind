...
Open http://127.0.0.1:4330. The default upstream is the public Remind backend. There is no sample data: actions affect the selected real organization or sandbox. Choose **Continue with IAm**. Remind sends you to the configured IAm login page with only `app_id=tos>remind` and its own callback URI. The user selects authorized organizations inside IAm. IAm returns an unscoped SLT that the gateway exchanges server-side, discovers the current authorized organization list, and opens the workspace. The Organization selector switches only between those granted workspaces. Settings provides **Continue with IAm** to change identity or organization grants. Sandbox sign-in accepts a test SLT without asking for an organization. A Carbon can browse reminders; a Silicon can manage its own reminders and webhook. Test identities must use the IAm sandbox linked to the selected Remind test environment.

## Supported workflows

- Browser redirect sign-in through IAm, plus independently authenticated test sessions; sandbox SLT login, automatic refresh, identity verification, logout and organization changes.
